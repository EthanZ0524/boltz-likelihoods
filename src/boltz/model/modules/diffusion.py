...
class AtomDiffusion(Module):
    ...
    def langevin(
        ...
        **network_condition_kwargs,
        ...
        langevin_args : dict
            ...
            replicates : int
                The number of trajectories to run for each provided 
                starting PDB/each structure generated via the diffusion
                rollout/PFODE sampling.
        
        multiplicity : int
            If no input PDBs are provided, multiplicity will control the
            number of trajectory starting structures, ie. the number of
            structures generated by the structure prediction head via
            diffusion or PFODE sampling.

        input_coords : dict
            Keys are PDB filenames, values are tensors of shape 
            (n_padded_atoms, 3)
            Processed centered coordinates of the input PDB(s) in 
            angstroms.
        ...
        replicates = langevin_args["replicates"]
        model_cache = {} if self.use_inference_model_cache else None

        if input_coords is not None:
            # Processing input_coords dict
            all_coords = []
            for _, struct_coords in input_coords.items():
                all_coords.append(struct_coords)
            input_coords = torch.stack(all_coords, dim=0)
            
            atom_coords = input_coords.repeat_interleave(
                replicates, 0
            )
            atom_mask = atom_mask.repeat_interleave(replicates * len(input_coords), 0) # TODO: there is no need for this or the equivalent line in the else block. The resizing is only used for shape - can be cleaner.
        ...
        # This is the final, nearly-denoised coordinate set.
        ...
        num_trajs = len(atom_coords_curr)
        ...
        for pdb_name, struct in tqdm(
            ...
            def ode_fn(sigma, x):
                ...
                with torch.enable_grad():
                    centered_struct = x[0] - x[0].mean(dim=0, keepdim=True).to(self.device) # Centering to origin.
                    ...
                    if likelihood_args['likelihood_mode'] == 'jac':
                        ...
                        score = _score_fn(centered_struct, sigma)[0] # (n_padded_atoms, 3)
                        update = (score * -sigma).detach()                        
                        estimates = []
                        ...
                        return update, -sigma * mean.detach() # (n_padded_atoms, 3), torch scalar.
    ...
    def calc_likelihoods_parallel_to(
        ...
        **network_condition_kwargs,
        ...
        """Does exact likelihood calculation using PFODE integration. 
        Parallelized using vmap and torchode
        Parameters
        ----------
        input_coords : dict
            Keys are PDB filenames, values are tensors of shape 
        ...
        def divergence_and_score_fn_single(t, x):
            ...
            # so we need to call jac(x, t) not jac(t, x)
            ...
            if likelihood_args['likelihood_mode'] == 'jac':
                ...
                for j in range(likelihood_args['hutchinson_samples']):
                    v = torch.randint(
                        low=0,
                        high=2,
                        size=centered_struct.shape,
                        generator=generator,
                        device=centered_struct.device,
                        dtype=centered_struct.dtype
                    ) * 2 - 1
        ...
        for i in tqdm(
            ...
            # sol.ys shape (batch, timepoints, features)
